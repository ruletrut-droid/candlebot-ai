  ...
  if (!signal) {
    return (
      <div className="terminal-card p-6 flex flex-col items-center justify-center min-h-[200px]">
        ...
        <p className="text-muted-foreground font-mono text-sm">Aguardando sinal...</p>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-3xl font-bold font-mono ${c.color}`}>{c.label}</h2>
              <p className="text-sm text-muted-foreground">{signal.asset} • {signal.price}</p>
            </div>
          </div>
          <div className="text-right">
            <p className={`text-2xl font-bold font-mono ${c.color}`}>{signal.probability}%</p>
